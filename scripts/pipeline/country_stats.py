...
def find_city_files(country_path: Path) -> list[Path]:
    """Find city Excel files in state subfolders (not state aggregates)."""
    city_files = []
    ...
    for state_dir in country_path.iterdir():
        ...
        # Find city files within each state folder
        for xlsx in state_dir.glob("*.xlsx"):
            # Skip temp files
            if xlsx.name.startswith("~"):
                continue
            # Skip stats files (state aggregates)
            if "stats" in xlsx.name.lower():
                continue
            # Skip files named after the state folder (e.g., Florida.xlsx in Florida/)
            state_name_lower = state_dir.name.lower()
            file_stem_lower = xlsx.stem.lower()
            if file_stem_lower == state_name_lower:
                continue
            # Skip common aggregate file patterns
            if file_stem_lower in ("all", "combined", "aggregate", "total"):
                continue

            city_files.append(xlsx)
...
def extract_stats_from_excel(xlsx_path: Path) -> dict:
    ...
    stats = {
        "city": xlsx_path.stem,  # Use filename without extension as city name
        "state": xlsx_path.parent.name,  # Parent folder is the state
        "file": xlsx_path.name,
        "leads_count": 0,
        "hotels_scraped": 0,
        "with_website": 0,
        ...
        "with_phone": 0,
        "with_email": 0,
        "booking_engines": defaultdict(int),  # Count by booking engine
    }
    
    try:
        wb = load_workbook(xlsx_path, read_only=True, data_only=True)
        
        # Get leads count and booking engine breakdown from Leads sheet
...
def generate_country_stats(country_name: str, city_stats: list[dict]) -> Path:
    ...
    total_engines = sum(all_engines.values())

    for i, (engine, count) in enumerate(sorted_engines[:10]):  # Top 10 engines
        r = 10 + i
        pct = round(count / total_engines * 100, 1) if total_engines else 0
    ...
    # Headers
    headers = ["City", "Top 25", "Scraped", "Booking Found", "Tier 1", "Tier 2"]
    ...
    for i, city in enumerate(sorted(city_stats, key=lambda x: x["booking_found"], reverse=True)):
        ...
        ws.cell(row=r, column=3, value=city["hotels_scraped"]).border = thin_border
