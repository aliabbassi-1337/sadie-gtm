...
from services.leadgen.dbpr_scraper import (
    DBPRIngester,
    DBPRLicense,
    DBPRIngestStats,
    LICENSE_TYPES,
...
class Service(IService):
    ...
    # =========================================================================
    # DBPR LICENSE INGESTION
    # =========================================================================

    async def ingest_dbpr_licenses(
        self,
        ...
        for i, lic in enumerate(licenses):
            ...
            try:
                ...
                hotel_dict = {
                    ...
                    "state": lic.state,
                    "country": "USA",
                    "phone_google": lic.phone,
                    "source": f"dbpr_{lic.license_type.lower().replace(' ', '_')}",
                    "status": 0,  # Pending
                }

                # Insert (will skip duplicates based on name+address)
                await repo.insert_hotel(
                    name=hotel_dict["name"],
                    website=None,  # DBPR doesn't have websites
                    source=hotel_dict["source"],
                    status=hotel_dict["status"],
                    address=hotel_dict.get("address"),
                    city=hotel_dict.get("city"),
                    state=hotel_dict.get("state"),
                    phone_google=hotel_dict.get("phone_google"),
                )
                saved += 1
