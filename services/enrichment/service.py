...
Uses shared lib/rms for RMS operations.
"""
...
class RMSRepo:
    ...
    async def get_hotels_needing_enrichment(
        self, limit: int = 1000
    ) -> List[RMSHotelRecord]:
    ...
    async def update_enrichment_status(self, booking_url: str, status: str) -> None:
        async with get_conn() as conn:
            await queries.update_rms_enrichment_status(
                conn, booking_url=booking_url, status=status
            )

    async def get_stats(self) -> Dict[str, int]:
        booking_engine_id = await self.get_booking_engine_id()
        async with get_conn() as conn:
            result = await queries.get_rms_stats(
                conn, booking_engine_id=booking_engine_id
            )
            if result:
                return dict(result)
            return {
...
class RMSQueue:
    ...
    def receive_messages(self, max_messages: int = 10) -> List[QueueMessage]:
        ...
        for msg in raw_messages:
            ...
            messages.append(
                QueueMessage(receipt_handle=msg["receipt_handle"], hotels=hotels)
            )
        return messages
...
class IService(ABC):
    ...
    async def calculate_customer_proximity(
        self, limit: int = 100, max_distance_km: float = 100.0
    ) -> int:
        pass
    ...
    async def enrich_rms_hotels(
        self, hotels: List[RMSHotelRecord], concurrency: int = 6
    ) -> EnrichResult:
        pass

    @abstractmethod
    async def enqueue_rms_for_enrichment(
        self, limit: int = 5000, batch_size: int = 10
    ) -> EnqueueResult:
        pass
...
class Service(IService):
    ...
    async def enrich_room_counts(
        self, limit: int = 100, free_tier: bool = False, concurrency: int = 15
    ) -> int:
        ...
        mode = (
            ...
            if free_tier
            else f"paid tier ({concurrency} concurrent)"
        )
        log(f"Claimed {len(hotels)} hotels for enrichment ({mode})")

        async def process_hotel(
            client: httpx.AsyncClient, hotel, semaphore: asyncio.Semaphore = None
        ):
            if semaphore:
    ...
    async def calculate_customer_proximity(
        self, limit: int = 100, max_distance_km: float = 100.0, concurrency: int = 20
    ) -> int:
        ...
        if not hotels:
            ...
            return 0
        proximity_log(
            f"Processing {len(hotels)} hotels for proximity calculation (concurrency={concurrency})"
        )
        semaphore = asyncio.Semaphore(concurrency)
        processed_count = 0
        ...
        async def process_hotel(hotel):
            ...
            async with semaphore:
                nearest = await repo.find_nearest_customer(
                    hotel_id=hotel.id, max_distance_km=max_distance_km
                )
                if nearest:
                    ...
                    proximity_log(
                        f"  {hotel.name}: nearest customer is {nearest['customer_name']} ({round(nearest['distance_km'], 1)}km)"
                    )
                    processed_count += 1
                else:
                    await repo.insert_customer_proximity_none(hotel_id=hotel.id)
                    proximity_log(
                        f"  {hotel.name}: no customer within {max_distance_km}km"
                    )

        await asyncio.gather(*[process_hotel(h) for h in hotels])
        proximity_log(
            f"Proximity calculation complete: {processed_count}/{len(hotels)} hotels have nearby customers"
        )
    ...
    async def enrich_websites(
        ...
        hotels = await repo.claim_hotels_for_website_enrichment(
            limit=limit, source_filter=source_filter, state_filter=state_filter
        )
        if not hotels:
            log("No hotels found needing website enrichment")
            return {"total": 0, "found": 0, "not_found": 0, "errors": 0, "api_calls": 0}
        log(
            f"Claimed {len(hotels)} hotels for website enrichment (concurrency={concurrency})"
        )
        ...
        async def process_hotel(hotel: dict):
            ...
            async with semaphore:
                ...
                if completed % 50 == 0:
                    log(
                        f"  Progress: {completed}/{len(hotels)} ({found} found, {skipped_chains} chains skipped)"
                    )

        await asyncio.gather(*[process_hotel(h) for h in hotels])
        log(
            f"Website enrichment complete: {found} found, {not_found} not found, {skipped_chains} chains skipped, {errors} errors"
        )
        return {
            "total": len(hotels),
            "found": found,
        ...
        async with async_playwright() as p:
            ...
            async def enrich_one(hotel: RMSHotelRecord, idx: int) -> tuple[bool, bool]:
                async with semaphore:
                    ...
                    url = (
                        hotel.booking_url
                        if hotel.booking_url.startswith("http")
                        else f"https://{hotel.booking_url}"
                    )
                    slug = url.split("/")[-1]
                    ...
                    if data and data.has_data():
                        ...
                        await self._rms_repo.update_enrichment_status(
                            hotel.booking_url, "enriched"
                        )
                        logger.info(f"Enriched {hotel.hotel_id}: {data.name}")
                        return (True, True)
                    else:
                        await self._rms_repo.update_enrichment_status(
                            hotel.booking_url, "no_data"
                        )
                        return (True, False)
    ...
    async def consume_rms_enrichment_queue(
        ...
        while not should_stop():
            ...
            logger.info(
                f"Progress: {hotels_processed} processed, {hotels_enriched} enriched"
            )
    ...
    async def get_rms_hotels_needing_enrichment(
        self, limit: int = 1000
    ) -> List[RMSHotelRecord]:
