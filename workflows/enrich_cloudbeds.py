...
async def extract_from_cloudbeds_page(page: Page) -> Dict[str, Any]:
    ...
    try:
        widget_data = await page.evaluate("""
            () => {
                ...
                if (!container) {
                    ...
                    if (addressHeading) {
                        // Get the parent container that holds both heading and content
                ...
                // Get all paragraph text (try multiple selectors for different page versions)
                ...
                const lines = paragraphs
                    ...
                    .filter(t => t && t.length > 0 && t.length < 100);  // Filter out empty and long text
        ...
        if widget_data and widget_data.get('lines') and len(widget_data['lines']) >= 2:
            ...
            # Parse lines to find address, city, state/country
            ...
            # Fallback: try original rsplit approach if pattern didn't match
            if 'state' not in result and len(lines) > 2:
                state_country = lines[2].strip()
                parts = state_country.rsplit(' ', 1)
                if len(parts) == 2 and len(parts[1]) <= 3:  # Country code is 2-3 chars
                    result['state'] = parts[0].strip()
                    country = parts[1].strip().upper()
                    result['country'] = 'USA' if country in ['US', 'USA'] else country
            
            # Phone from tel link (more reliable) or pattern match
